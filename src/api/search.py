...
def metrics_endpoint():
    ...
    text = metrics.render()
    try:  # live ingest state: pending/queued/embedding (queue depth) + indexed
        with db.pool().connection() as conn:
            rows = conn.execute(
                "SELECT kind, status, count(*) AS n FROM ms_videos GROUP BY kind, status"
            ).fetchall()
        text += "# TYPE glimpse_sources gauge\n"
        for r in rows:
            text += f'glimpse_sources{{kind="{r["kind"]}",status="{r["status"]}"}} {r["n"]}\n'
    except Exception:
        pass  # metrics must never 500
...
def ask(req: AskRequest, x_user_id: str | None = Header(default=None)):
    ...
    try:
        ...
        from .. import metrics
        metrics.inc("glimpse_rate_limited_total")
...
def retrieve_only(req: AskRequest, x_user_id: str | None = Header(default=None)):
    ...
    try:
        ...
        raise HTTPException(429, "Rate limit exceeded — please slow down.",
                            headers={"Retry-After": str(int(e.retry_after) + 1)})
    return rag_search.retrieve(req.question.strip(), uid, top_k=req.top_k,
                               video_id=req.video_id, video_ids=req.video_ids or None)
...
def _render(mode: str) -> str:
    ...
    if not index.exists():
        return "<h1>Glimpse</h1><p>ui/index.html not found.</p>"
